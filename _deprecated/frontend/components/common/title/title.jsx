...
class Title extends PureComponent {
	...
	next = () => {
		...
		if (this.active === this.props.titles.length - 1) next = 0;
		...
		let tl = new TimelineLite()
			...
			.staggerTo(
				this.projects[this.active].current.childNodes,
				0.75,
				{ y: '-100%', opacity: 0, ease: Power2.easeInOut },
				0.02,
				0
			)
			.to(this.projects[next].current, 0.75, { y: '0%', ease: Power2.easeInOut }, 0)
			.staggerFrom(
				this.projects[next].current.childNodes,
				0.75,
	...
	prev = () => {
		...
		if (this.active === 0) prev = this.props.titles.length - 1;
		...
		let tl = new TimelineLite()
			...
			.staggerFrom(
				this.projects[prev].current.childNodes,
				0.75,
				{ y: '-100%', ease: Power2.easeInOut },
				0.02,
				0
	...
	render() {
		...
		if (this.props.isTablet) dynamicStyles = tabletStyles;
		else if (this.props.isMobile) dynamicStyles = mobileStyles;
			...
			component = (
				<React.Fragment>
					...
					<span
						...
						className='spectral unclickable'
						id='container'>
						<span className='title-parent' key={this.props.title}>
							{this.props.title.split('').map((letter, index) => {
								...
								if (letter === ' ') classes += ' hover-link-space';
