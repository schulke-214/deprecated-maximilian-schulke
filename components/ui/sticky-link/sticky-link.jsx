...
import './sticky-link.scss';

class StickyLink extends PureComponent {
	...
	handleMouseEnter = ev => {
		...
		if (!this.props.device.meta.mobile) {
			if (this.props.virtualCursor) this.props.virtualCursor.current.shrink();
	...
	handleMouseOut = ev => {
		...
		if (!this.props.device.meta.mobile) {
			if (this.props.virtualCursor) this.props.virtualCursor.current.reset();
	...
	render() {
		return (
			<div className='sticky-link ev__all' style={this.props.style}>
				<span className='sticky-link__content' ref={this.content}>
					{this.props.children}
				</span>
				<div
					className='sticky-link__trigger ev__all'
...
export default connect(mapStateToProps)(StickyLink);
