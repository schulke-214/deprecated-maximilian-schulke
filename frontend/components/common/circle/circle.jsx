...
class Circle extends Component {
	...
	next = next => {
		if (next === 1) this.lastToFirst();
		else
			TweenLite.to(this.circle.current, 0.5, {
				strokeDashoffset: this.circleLength - this.step * next,
				ease: Power2.easeInOut
			});
	};

	prev = prev => {
		if (prev === this.props.length) this.firstToLast();
		else
			TweenLite.to(this.circle.current, 0.5, {
				strokeDashoffset: this.circleLength - this.step * prev,
				ease: Power2.easeInOut
			});
	};
	...
	lastToFirst = () => {
		TweenLite.to(this.circle.current, 0.5, {
			...
			onComplete: () => {
				...
				TweenLite.to(this.circle.current, 0.25, {
					strokeDashoffset: this.circleLength - this.step,
					ease: Power2.easeOut
				});
			}
		});
					...
					<svg viewBox='0 0 54 54'>
						...
						<path
							...
							style={{
								...
								strokeDasharray: '158.1px, 168.1px'
							}}
						/>
					</svg>
				</span>
